...
| 12 | Social layer | 🟡 | `scripts/import_social.py` ingests **public** Bluesky (`searchPosts`) and Mastodon (tag timelines) posts for the configured hashtag variants (`#unosw`, `#UNOpenSourceWeek`, `#OpenSourceWeek`, per-year, `#OSPOsForGood`, plus the phrase "UN Open Source Week" — in `conferences/<id>.json` `social`), storing metadata + permalink + short excerpt into `data/<id>/social.json`. Throttled, idempotent, no private content; surfaced as a distinct "social" search category and `/api/social.json`, never as authoritative fact. Runs via the weekly **Ingest** GitHub Action — the agent/CI sandbox denies egress to these hosts (`403 CONNECT`), so ingestion happens on GitHub-hosted runners. |
| 13 | Preservation | 🟡 | Community draft transcripts (cleaned auto-captions) for the recorded plenary sessions are stored under `conferences/<year>/` and linked from the matching session pages beside their authoritative UN Web TV recording. `scripts/import_wayback.py` (weekly **Ingest** Action) collects the platform's external authoritative links, checks the Wayback Machine, and submits missing ones to Save Page Now — writing snapshot URLs/status to `data/<id>/preservation.json` (exposed at `/api/preservation.json`). Throttled and idempotent; needs network egress, so it runs on GitHub-hosted runners. Surfacing archived links on the session pages themselves is the remaining step. |
...
| 15 | Research outputs | ✅ | Generated briefings under `/reports/`: an **annual briefing** per conference-year (stats, sessions by day, themes, organizations, recordings, quotes); a **cross-year theme briefing** per topic (sessions per year, organizations and people active, a standards/references reading list, quotes); and cross-year **directories** — every **organization** and every **person** (by year, linked to their profiles) plus a **repositories & standards index** (projects + referenced standards/frameworks). All aggregated from the datasets and linked back to the records; per-session and per-day views are covered by the session pages and the annual "sessions by day" breakdown. |
| 16 | WebMCP | 🟡 | `/api/index.json` is a single discovery document: a `discovery` block linking every machine-readable surface (search index, merged knowledge graph, reports manifest, timeline, relationship map, `llms.txt`), per-conference `event_state` (archive vs upcoming), and per-year entries (dataset URLs, knowledge graph, annual report). `/api/reports.json` enumerates every annual/theme/directory briefing, and `/llms.txt` mirrors it all — so an AI system retrieves sessions/topics/quotes/people/orgs/repos without crawling HTML. A hosted MCP-server endpoint (vs. these static resources) is the remaining piece. |
